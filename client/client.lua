	...
	while true do
		...
		if location ~= nil and hosDuration > 0 then
			...
			if distance > 20 then
				...
				ESX.ShowNotification("Your hospital stay time was extended as you were not officially discharged.")
				ESX.ShowNotification('You have ' .. hosDuration .. ' seconds left in hospital.')
			end
...
AddEventHandler("bixbi_hospital:send", function(duration, inputLocation)
	...
	if loc ~= nil then
		...
		if DoesEntityExist(playerPed) then
			...
			ESX.ShowNotification('You have been sent to ' .. loc.label .. ' for ' .. duration .. ' seconds.')
...
AddEventHandler("bixbi_hospital:release", function()
	...
	ESX.ShowNotification('You have been released from ' .. location.label .. ' .')
